...
public class DataManager {
   //Variables
            ...
            for (int i=0; i<totalNumberOfColsInRow(currentRow);i++){
                if (workSheet.getRow(currentRow).getCell(i) != null){
                    ...
                    //else if(workSheet.getRow(currentRow).getCell(i).getCellType() == CellType.STRING){
                    	else if(workSheet.getRow(currentRow).getCell(i).getCellTypeEnum() == CellType.STRING){
                        getvalue = workSheet.getRow(currentRow).getCell(i).getStringCellValue();
                        dataList.add (getvalue);
                    } else if (workSheet.getRow(currentRow).getCell(i).getCellTypeEnum()== CellType.NUMERIC){
    ...
    ////////////////////////////////////////////////////////////////////////
    //Method:ExcelInsert
    //      The method will inseart data into cell as a String
    //      and return the same value. The method works for both xls or xlsx
    //      files.
    /////////////////////////////////////////////////////////////////////////
    public static String ExcelInsert(String inputfile, int sheetnum, int rownum, int colnum, String inputdata) throws IOException {
        String returnValue = null;
        String fileName;
        Workbook workbookObject = null;
        FileInputStream fileObject = null;
        ...
        try{
            ...
            workbookObject = WorkbookFactory.create(fileObject);
            workSheet = workbookObject.getSheetAt(sheetnum);
            totalnumbeofrows = workSheet.getLastRowNum();
            //Creat or get row.
            if (totalnumbeofrows < rownum){
                row = workSheet.createRow(rownum);
            }else{
                row = workSheet.getRow(rownum);
            }
            //Create or get cell type and value
            cell = row.getCell(colnum);
            if (cell == null) {
            ...
            workbookObject.write(fileOutputStream);
            returnValue = getCellString(cell);
