...
/**
 * Reporter designed to render self-contained HTML top down view of a testing
 * suite.
 */
public class EmailableReporter implements IReporter {
    ...
    public void generateReport(List<XmlSuite> xml, List<ISuite> suites, String outdir) {
        ...
        for (ISuite suite : suites) {
            ...
            for (ISuiteResult r2 : r.values()) {
                ITestContext context = r2.getTestContext();
                passedtest = passedtest + context.getPassedTests().size();
                failedtest = failedtest + context.getFailedTests().size();
                skippedtest = skippedtest + context.getSkippedTests().size();
            }
        }
        if (ApplicationProperties.EMAIL_EXECUTION_REPORT.getStringVal().equals("true")) {
            postExecutionAction(suites, passedtest, failedtest, skippedtest);
        }

    }

    private void postExecutionAction(List<ISuite> suites, int passedtest, int failedtest, int skippedtest) {
        EmailExecutionReport.sendEmailWithAttachedExecutionReport(suites, passedtest, failedtest, skippedtest);
    }

    protected PrintWriter createWriter(String outdir) throws IOException {
        return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, "delta-automation-emailable-report.html"))));
    }
    ...
    public void generateSuiteSummaryReport(List<ISuite> suites) {
        tableStart("param");
        m_out.println("<tr><center><b>DeltaDental Custom TestNG Test Automation Report" + " " + getDateInStringFormat(new Date()) + "</b></center></tr> ");
        m_out.println("<tr><th>Test</th>");
    ...
    protected void startHtml(PrintWriter out) {
        ...
        out.println("<head>");
        out.println("<title>TestNG: Delta Dental Automation Test</title>");
